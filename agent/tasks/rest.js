...
function estimatedHeal(player) {
  ...
  const amount = Math.min(missing, Math.max(0, Math.floor(maxHp * 0.3)));
...
function healScore(player, threats) {
  ...
  if (hp < 0.35) score += 18;
  else if (hp < 0.5) score += 10;
  else if (hp < 0.65) score += 4;
...
function rankSmithCards(cards) {
  const list = Array.isArray(cards) ? cards : [];
  return list
    .map((card, index) => ({
      card,
      index,
      score: smithScore(card),
      why: smithReason(card),
...
function explainRest(choice, heal, best, threats, hp) {
  if (choice.action === 'HEAL') {
    if (hp < 0.4) return `生命偏低，先回血到 ${heal.healTo}。后面再找火堆升级。`;
  ...
  if (hp >= 0.75 && heal.missing < 12) return `生命还够，建议升级「${name}」。${best.why}。`;
...
function promptPayload(state) {
  ...
  return {
    ...
    candidates: [
      ...
      ...smiths.slice(0, 4).map((item, offset) => ({
        index: offset + 1,
        action: 'SMITH',
        cardId: item.card.id,
        name: item.card.name,
  ...
  if (llm?.enabled && typeof llm.completeRest === 'function') {
    try {
      ...
      if (option?.action === 'SMITH') {
        const card = smiths.find(item => item.card.id === option.cardId) || smiths[index - 1];
